...
These artifacts were produced by real Ultralytics YOLO11 and project-pipeline
runs on 2026-07-29. They use staged, non-patient images only.
...
Both one-epoch YOLO11s fit checks succeeded before the candidate runs:
...
The larger medium model did not improve validation recall, so it was not
promoted. The complete machine-readable comparison is
...
The tube class had no instances and therefore has no reported metric.
...
Rack_D was originally grouped away from training, but its baseline failures
informed this retraining progression. It is therefore diagnostic held-out
evidence, not a pristine final test. A new physical rack group is required for
an unbiased future final evaluation.
...
This is a possible issue requiring human review. The prototype must not be used
to approve a rack, make a clinical decision, or replace human inspection.
