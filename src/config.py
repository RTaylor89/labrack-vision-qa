...
Why do I have a config file at all?
    ...
    codebase. Instead I put them here once and every other module imports them.
...
#   I treat them as noise. 0.25 is a common, forgiving starting point.
...
# --- Classes ---------------------------------------------------------------
# The object types this project actually cares about. I keep them in a tuple
...
# --- Input validation ------------------------------------------------------
# The only image types I accept. I check a file's extension against this list
# before trying to open it, so I can give a clear error instead of crashing.
SUPPORTED_EXTENSIONS = (".jpg", ".jpeg", ".png")


# --- Annotation colors -----------------------------------------------------
# Colors I use when I draw boxes on the image. NOTE: OpenCV orders colors as
# (Blue, Green, Red), NOT the usual (Red, Green, Blue). This trips up almost
# everyone the first time, so I spell it out here.
CLASS_COLORS = {
...
# If I ever detect a class that is not in the dict above (e.g. a COCO class
# from the pretrained model), I fall back to grey instead of crashing.
...
# prototype, so I say so clearly and consistently, everywhere.
DISCLAIMER = (
...
def output_paths(image_path, output_dir=DEFAULT_OUTPUT_DIR):
    ...
    I keep this here (in config) so the naming rule lives in one place. If I
    ever want to rename outputs, I change it once and every caller follows.
    ...
    I then build three sibling filenames inside output_dir and return them in
