...
Design idea:
    I deliberately keep "running the model" separate from deciding what the
    results mean. That second part will live in qa_rules.py. This separation has
    two big payoffs:
      1. I can test the QA logic without ever loading a heavy model.
      2. If I swap YOLO for something else later, only this file changes.

What this file produces:
    A "detection" is just a plain Python dictionary. I use a dict (instead of a
...
from . import config  # my project settings module


def validate_image_path(image_path):
    """
    Make sure the file is one I can actually process, BEFORE I load a model.

    I check three things in order and raise a clear, specific error for each:
      - the file exists,
      - it is a file (not a folder),
      - its extension is one I support.
...
def _load_model(model_path):
    ...
    slow. By importing it only when I truly load a model, the rest of the code
    (and all my QA tests) can run without that heavy dependency installed.
...
class RackDetector:
    ...
    and I only want to do it once. I load it in __init__ (when the object is
    ...
    def __init__(self, model_path=config.MODEL_PATH,
        ...
        # I remember the settings so I can record them later in the report.
    ...
    def detect(self, image_path):
        ...
        Steps:
          ...
          2. Ask the model to predict, passing my confidence threshold.
          3. Walk through each detected box and copy the useful bits into my
             own plain-dict format.
          4. Return everything the rest of the pipeline needs.
        """
        path = validate_image_path(image_path)

        # predict() returns a list (one entry per image). I gave it one image,
        # so I take element [0]. verbose=False keeps it from printing to screen.
        ...
        if boxes is not None:
            for box in boxes:
                ...
                # Here I convert each to a whole-number pixel coordinate.
